...
router.post('/package-zip', async (req, res) => {
  ...
  if (!Array.isArray(files)) {
    return res.status(400).json({ ok: false, error: { code: 'BAD_REQUEST', message: 'Invalid files' } })
  }
  try {
    ...
    return res.status(500).json({ ok: false, error: { code: 'ZIP_FAILED', message: e.message || 'Zip failed' } })
  }
})
