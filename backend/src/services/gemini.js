...
export async function generateFilesFromPrompt(prompt) {
  ...
  if (!gen) {
    return {
      files: fallbackTemplates(prompt),
      testResults: [],
      qualityReport: { maintainability: 0.7, complexity: 0.3, coverage: 0.5 },
    }
  }
  try {
...
export async function patchFilesWithRequest(featureRequest, currentFiles) {
  ...
  try {
    ...
    const raw = resp.response.text()
    const json =
      safeJsonParse(raw) || safeJsonParse(raw.replace(/```(json)?/g, '').trim())
...
function reactTemplate() {
  return [
    {
      path: 'index.html',
      content: `<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Generated App</title><script defer src="/main.js"></script><link href="https://cdn.jsdelivr.net/npm/tailwindcss@3.4.10/dist/tailwind.min.css" rel="stylesheet"></head><body class="bg-gray-50"><div id="root"></div></body></html>`,
    },
    {
      path: 'main.js',
      content: `const e=React.createElement;const root=ReactDOM.createRoot(document.getElementById('root'));function App(){const [items,setItems]=React.useState([]);const [text,setText]=React.useState('');return e('div',{className:'max-w-xl mx-auto p-6'},[e('h1',{className:'text-2xl font-bold mb-4'},'Hello from AppGenieX'),e('div',{className:'flex gap-2 mb-4'},[e('input',{className:'border flex-1 p-2 rounded',value:text,onChange:e=>setText(e.target.value),placeholder:'Add todo...'}),e('button',{className:'bg-blue-600 text-white px-3 rounded',onClick:()=>{if(text.trim()) setItems([...items,{id:Date.now(),text}]);setText('')}},'Add')]),e('ul',{className:'space-y-2'},items.map(it=>e('li',{key:it.id,className:'p-2 bg-white rounded border'},it.text)))])}root.render(e(React.StrictMode,null,e(App)))`,
    },
  ]
}
  ...
  return [
    {
      ...
      content: `<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>Full-Stack App</title><script defer src="/main.js"></script><link href="https://cdn.jsdelivr.net/npm/tailwindcss@3.4.10/dist/tailwind.min.css" rel="stylesheet"></head><body><div id="root"></div></body></html>`,
    },
    {
      path: 'frontend/main.js',
      content: `const e=React.createElement;const root=ReactDOM.createRoot(document.getElementById('root'));function App(){const [msg,setMsg]=React.useState('');React.useEffect(()=>{fetch('http://localhost:5000/health').then(r=>r.json()).then(d=>setMsg('Backend: '+(d.data?.status||'ok')))},[]);return e('div',{className:'p-6'},[e('h1',{className:'text-xl font-bold'},'Full-Stack Template'),e('p',null,msg)])}root.render(e(React.StrictMode,null,e(App)))`,
    },
    {
      path: 'backend/server.js',
      content: `import express from 'express';const app=express();app.get('/health',(req,res)=>res.json({ok:true,data:{service:'template-backend',status:'healthy'}}));app.listen(5000);`,
    },
  ]
}
